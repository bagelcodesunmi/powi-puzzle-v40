...
function scanSeeds(label, level, budget, base, target, range, timeMs) {
  const found = [];
  let tried = 0;
  process.stdout.write(`\n[${label}] 예산${budget} 탐색 중...\n`);
  for (let t = 1; t <= range && found.length < target; t++) {
    const seed = base + t;
    tried++;
    const r = solve(level, seed, budget, timeMs);
    process.stdout.write(`\r  ${tried}/${range} 시도, 통과 ${found.length}/${target}  `);
    if (r.status === 'PASS') found.push(seed);
  }
  const rate = Math.round(found.length / tried * 100);
  console.log(`\n  결과: [${found.join(',')}]  통과율 ~${rate}%`);
  return { found, rate };
}

// ── Lv12 후보: 5×5 2기 전투 — 코너 대각 [[0,0],[4,4]]
console.log('\n===== Lv12: 5×5 2기 전투 (코너 대각) =====');
const lv12 = { board:5, enemies:[[0,0],[4,4]], spread:true, combat:true };
scanSeeds('Lv12', lv12, 9, 1200000, 5, 60, 8000);

// ── Lv13 후보: 5×5 2기 전투 — 내부 대각 [[1,1],[3,3]]
console.log('\n===== Lv13: 5×5 2기 전투 (내부 대각) =====');
const lv13 = { board:5, enemies:[[1,1],[3,3]], spread:true, combat:true };
scanSeeds('Lv13', lv13, 10, 1300000, 5, 60, 8000);

// ── Lv14 후보: 6×6 단일 전투 브릿지 — 코너(0,0), 예산 10~12
console.log('\n===== Lv14 브릿지: 6×6 단일 전투 (코너) =====');
const lv14b = { board:6, enemies:[[0,0]], spread:true, combat:true };
for (const budget of [10, 11, 12]) {
  const r = scanSeeds(`Lv14 예산${budget}`, lv14b, budget, 1400000+budget*10000, 5, 30, 10000);
  if (r.rate >= 40) { console.log(`  → 예산 ${budget} 채택!`); break; }
}
