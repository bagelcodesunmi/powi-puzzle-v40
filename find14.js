// 레벨 14 → 6×6으로 변경 시 예산별 통과율 스캔
// 적 위치는 기존 5×5 (2,1)을 6×6 기준으로 유지
...
// 6×6, 적 (2,1) — 기존 14번 위치 유지
const LEVEL14_6x6 = { board: 6, enemies: [[2,1]], spread: true, combat: true };
const SAMPLE = 30;

console.log(`레벨 14 → 6×6 (적 [2,1]) 예산별 통과율 스캔 (시드 ${SAMPLE}개 샘플)\n`);
console.log('예산 | 통과수 | 통과율 | 통과 시드');
console.log('-----|--------|--------|----------');

for (const budget of [8, 9, 10, 11]) {
  const passed = [];
  for (let t = 1; t <= SAMPLE; t++) {
    const seed = 1400000 + t;
    const r = solve(LEVEL14_6x6, seed, budget, 8000);
