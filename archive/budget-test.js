// 레벨 15의 특정 시드에 대해 예산별 PASS/FAIL 확인
// 실행: node budget-test.js
...
const BASE_LEVEL = { board: 6, enemies: [[0,0],[2,0]], hp: 2, spread: true, combat: true };
...
function solveFixed(level, seed, budget, timeMs = 30000) {
  const N = level.board;
  const NN = N * N;
  const idx = (x, y) => y * N + x;
  const inBoard = (x, y) => x >= 0 && x < N && y >= 0 && y < N;
  ...
  try {
    ...
    return { status: ok ? 'PASS' : 'FAIL', nodes };
  } catch (e) {
...
// 레벨 15 검증된 시드들에 대해 예산 6~9 범위 테스트
const seeds = [1500008];
const budgets = [6, 7, 8, 9];

console.log('레벨 15 시드별 예산 테스트\n');
for (const seed of seeds) {
  console.log(`시드 ${seed}:`);
  for (const budget of budgets) {
    const t0 = Date.now();
    const r = solveFixed(BASE_LEVEL, seed, budget, 30000);
    const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
    console.log(`  예산 ${budget}: ${r.status}  [${elapsed}s, 노드 ${r.nodes.toLocaleString()}]`);
    if (r.status === 'PASS') break; // min 찾으면 위쪽은 당연히 PASS
  }
}
