// 레벨 15: 예산별 통과율 스캔 (시드 20개 샘플)
// 목표: 5개 확보에 필요한 최소 예산 파악
...
function makeSpreadPicker(N, prng) {
  ...
  function isSuicide(grid, c) {
    const seen = new Set([c]); const q = [c]; let empties = 0;
    while (q.length) {
      const k = q.pop();
...
function solve(level, seed, budget, timeMs=8000) {
  ...
  for (let i=0;i<NN;i++) {
    ...
    for (const [dx,dy] of DIRS) { const nx=x+dx,ny=y+dy; if (nx>=0&&nx<N&&ny>=0&&ny<N) ns.push(idx(nx,ny)); }
    NB.push(ns);
  }
  const prng=mulberry32(seed);
  const fx=prng()<.5, fy=prng()<.5;
  ...
  function forEachRegion(g,cb) {
    ...
    for (let i=0;i<NN;i++) {
      ...
      const cells=[],q=[i]; seen[i]=1; let hasEnemy=false,empties=0;
      while (q.length) {
        const c=q.pop(); cells.push(c);
        if (g[c]===ENEMY) hasEnemy=true; else if (g[c]===EMPTY) empties++;
  ...
  function enemyTurn(g) {
    ...
    const g2=g.slice(); g2[next]=ENEMY; capture(g2);
    return [enemyCnt(g2)===0?null:g2];
  }
  function candidateMoves(g) {
    const out=[];
    forEachRegion(g,(cells,hasEnemy)=>{ if (hasEnemy) for (const c of cells) if (g[c]===EMPTY) out.push(c); });
    return out;
  }
  const memo=new Map();
  function win(g,stones) {
    ...
    for (const m of candidateMoves(g)) {
      const g2=g.slice(); g2[m]=level.hp||1; capture(g2);
      if (enemyCnt(g2)===0) { result=true; break; }
      const branches=enemyTurn(g2);
      let all=true;
  ...
  try {
    return { status: win(g0,budget)?'PASS':'FAIL', nodes };
  } catch(e) {
    if (e instanceof Timeout) return { status:'TIMEOUT', nodes };
    throw e;
  }
}

const LEVEL15 = { board:6, enemies:[[0,0],[2,0]], hp:2, spread:true, combat:true };
const SAMPLE = 20; // 시드 20개 샘플

console.log(`레벨 15 — 예산별 통과율 스캔 (시드 ${SAMPLE}개 샘플, 시드당 8초)\n`);
console.log('예산 | 통과 | 통과율 | 통과 시드');
console.log('-----|------|--------|----------');

for (const budget of [9, 10, 11, 12]) {
  const passed = [];
  for (let t = 1; t <= SAMPLE; t++) {
    const seed = 1500000 + t;
    const r = solve(LEVEL15, seed, budget, 8000);
    if (r.status === 'PASS') passed.push(seed);
    process.stdout.write(`\r예산 ${budget}: ${t}/${SAMPLE} 시도, 통과 ${passed.length}개  `);
  }
  console.log(`\n  ${budget}    |  ${passed.length}/${SAMPLE}  |  ${Math.round(passed.length/SAMPLE*100)}%   | [${passed.join(',')}]`);
}
