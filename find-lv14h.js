// 레벨 14.5 후보 탐색 — 6×6, 단일 적, 전투, 목표 통과율 50~60%
// 적 위치 후보 × 예산 9~10 조합 스캔
...
function solve(level,seed,budget,timeMs=6000){
...
console.log('Lv14.5 후보 탐색 — 6×6 단일 적, 전투, 목표 통과율 50~60%\n');
console.log('위치       | 예산9 통과율 | 예산10 통과율 | 추천');
console.log('-----------|-------------|--------------|------');

for (const { pos, name } of CANDIDATES) {
  const level = { board:6, enemies:[pos], spread:true, combat:true };
  const results = {};
  for (const budget of [9, 10]) {
    let pass = 0;
    for (let t = 1; t <= SAMPLE; t++) {
      const seed = 1450000 + t;
      const r = solve(level, seed, budget, 6000);
      if (r.status === 'PASS') pass++;
    }
    results[budget] = Math.round(pass / SAMPLE * 100);
  }
  const r9 = results[9], r10 = results[10];
  const rec = (r9 >= 45 && r9 <= 65) ? `✅ 예산9 (${r9}%)` :
              (r10 >= 45 && r10 <= 65) ? `✅ 예산10 (${r10}%)` : '—';
  console.log(`${name.padEnd(10)} | ${String(r9+'%').padEnd(11)} | ${String(r10+'%').padEnd(12)} | ${rec}`);
}
