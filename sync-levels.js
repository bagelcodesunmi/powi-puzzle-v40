...
function gameLine(s) {
  ...
  if (s.grayCells && s.grayCells.length) p.push(`grayCells: ${xyArr(s.grayCells)}`);
...
function presetLine(s) {
  ...
  const gray = (s.grayCells && s.grayCells.length) ? `, grayCells: ${xyObj(s.grayCells)}` : '';
  return `  { name: '${s.name}', board: ${s.board}, combat: ${!!s.combat}, hp: ${s.hp || 1}, mirror: ${s.mirror !== false}, ` +
    ...
    `rightWalls: ${xyObj(s.rightWalls)}, downWalls: ${xyObj(s.downWalls)}${cracked}${gray} },`;
}
